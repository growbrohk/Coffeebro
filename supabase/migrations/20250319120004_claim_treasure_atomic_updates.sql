-- Update claim_treasure_atomic: add treasure time window and claim limit checks
...
begin
  ...
  -- Treasure time window checks
  if v_treasure.starts_at is not null and now() < v_treasure.starts_at then
    return query select 'NOT_YET_ACTIVE'::text, 'This treasure is not yet active'::text, null::jsonb;
    return;
  end if;

  if v_treasure.ends_at is not null and now() > v_treasure.ends_at then
    return query select 'TREASURE_ENDED'::text, 'This treasure has ended'::text, null::jsonb;
    return;
  end if;

  -- Claim limit check
  if v_treasure.claim_limit is not null then
    select count(*) into v_claim_count
    from public.hunt_claims
    where treasure_id = v_treasure.id;

    if v_claim_count >= v_treasure.claim_limit then
      return query select 'LIMIT_REACHED'::text, 'This treasure has reached its claim limit'::text, null::jsonb;
      return;
    end if;
  end if;
