-- Tasting vouchers: redeem using mint snapshot (vouchers.menu_item_id), not live item row.
-- Also restrict deleting tasting_package_items that vouchers still reference.

alter table public.vouchers
  drop constraint if exists vouchers_tasting_package_item_id_fkey;

alter table public.vouchers
  add constraint vouchers_tasting_package_item_id_fkey
    foreign key (tasting_package_item_id)
    references public.tasting_package_items (id)
    on delete restrict;
