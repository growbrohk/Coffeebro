...
function vouchersFromCampaign(c: CampaignWithVouchers): VoucherDraft[] {
  return (c.campaign_vouchers ?? []).map((cv, i) => ({
    ...
    redeem_starts_at: periodBoundFromSaved(cv.redeem_starts_at, "start"),
    redeem_ends_at: periodBoundFromSaved(cv.redeem_ends_at, "end"),
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      vouchers: vouchers.map((v, i) => ({
        ...
        redeem_starts_at:
          v.redemption_mode === "fixed_period"
            ? (periodBoundToInstant(v.redeem_starts_at, "start")?.toISOString() ?? null)
            : null,
        redeem_ends_at:
          v.redemption_mode === "fixed_period"
            ? (periodBoundToInstant(v.redeem_ends_at, "end")?.toISOString() ?? null)
            : null,
        quantity: v.quantity,
